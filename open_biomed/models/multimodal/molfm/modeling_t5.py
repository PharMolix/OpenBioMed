...
class T5Block(nn.Module):
    def __init__(self, config, layer_num, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        ...
        if self.is_decoder:
            ...
            self.has_encoder_attention = False
        elif layer_num % config.cross_attention_freq == 0:
            self.cross_attention_graph = T5LayerCrossAttention(config)
            self.has_encoder_attention = True
        else:
            self.has_encoder_attention = False
    ...
    def forward(
        ...
        do_cross_attention = (self.is_decoder or self.has_encoder_attention) and encoder_hidden_states is not None
        if do_cross_attention:
            ...
            if self.is_decoder:
                ...
                layer_module = self.cross_attention_graph
            cross_attention_outputs = layer_module(
                hidden_states,
