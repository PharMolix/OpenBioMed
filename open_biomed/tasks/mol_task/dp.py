...
def main(args, config):
    ...
    # _build
    save_path = ""
    if len(config["data"]["drug"]["modality"]) > 1 and "kg" in config["data"]["drug"]["featurizer"]:
        save_path = os.path.join(
            config["data"]["drug"]["featurizer"]["kg"]["save_path"], "dp-" + args.dataset + "_val.pkl")
    ...
    if args.mode == "train":
        ...
        logger.info("%s test %s=%.4lf" %
                    (args.dataset_name, metric_name, results[metric_name]))
    """
    logger.info(f"dp_{config['model']}_{args.dataset_name}_{args.seed}_{args.batch_size}.txt")
    with open(f"dp_{config['model']}_{args.dataset_name}_{args.seed}_{args.batch_size}.txt", "a") as f:
        # f.write(f"test {metric_name}={results[metric_name]} best_epoch {epoch} lr {args.lr} weight_decay {args.weight_decay} dropout {config['network']['structure']['drop_ratio']}  ckpts {config['network']['structure']['init_checkpoint']}\n ")
        f.write(f"test {metric_name}={results[metric_name]} best_epoch {epoch} lr {args.lr} weight_decay {args.weight_decay} \n ")
    """
